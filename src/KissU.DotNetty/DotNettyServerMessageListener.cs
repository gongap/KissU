            ...
            bootstrap
                ...
                .ChildHandler(new ActionChannelInitializer<IChannel>(channel =>
                {
                    ...
                    pipeline.AddLast(workerGroup, "ServerHandler", new ServerHandler(async (contenxt, message) =>
                    {
                        ...
                        if(contenxt.Channel.RemoteAddress is IPEndPoint ipEndPoint)
                        {
                            var remoteAddress = string.Concat(new string[] { ipEndPoint.Address.MapToIPv4().ToString(), ":", ipEndPoint.Port.ToString() });
                            RpcContext.GetContext().SetAttachment("PeerAddress",remoteAddress);
                        }
