...
namespace KissU.CPlatform
{
    ...
    public static class ServiceBuilderExtensions
    {
        ...
        public static IServiceBuilder AddServiceRuntime(this IServiceBuilder builder)
        {
            ...
            builder.ContainerBuilder.RegisterType(typeof(DefaultServiceExecutor)).As(typeof(IServiceExecutor)).Named<IServiceExecutor>(CommunicationProtocol.Rpc.ToString()).SingleInstance();
        ...
        {
            CheckHelper.NotNull(services, "services");
            // 注册服务ID生成实例
        ...
        private static IServiceBuilder AddRuntime(this IServiceBuilder builder)
        {
            ...
            containerBuilder.Register(provider => new AttributeServiceEntryProvider(types, provider.Resolve<IClrServiceEntryFactory>(), provider.Resolve<CPlatformContainer>())).As<IServiceEntryProvider>().SingleInstance();
            containerBuilder.Register(provider => new ManageServiceEntryProvider(types, provider.Resolve<IClrServiceEntryFactory>(), provider.Resolve<CPlatformContainer>())).As<IServiceEntryProvider>().SingleInstance();
        ...
        public static IHostBuilder AddMicroService(this IHostBuilder hostBuilder, Action<IServiceBuilder> option)
        {
            return  hostBuilder.ConfigureHostConfiguration(builder =>
            {
                ...
                builder.AddJsonFile("appsettings.json", optional: false, reloadOnChange: true)
                    ...
                    .AddCPlatformFile("servicesettings.json", false, true)
                    .AddCPlatformFile($"servicesettings.{environment}.json", false, true);
            })
                .ConfigureServices(services =>
                {
                    services.RegisterModules();
                })
                .ConfigureContainer(builder => builder.AddMicroService(option));
        }
                ...
                foreach (var assembly in referenceAssemblies)
                {
                    ...
                    containerBuilder.RegisterAssemblyTypes(assembly)
                        .Where(t => typeof(IServiceKey).GetTypeInfo().IsAssignableFrom(t) && t.IsInterface 
                            && t.GetTypeInfo().GetCustomAttribute<DisableConventionalRegistrationAttribute>() == null)
                    ...
                    containerBuilder.RegisterAssemblyTypes(assembly)
                        .Where(t => typeof(ISingletonDependency).GetTypeInfo().IsAssignableFrom(t) &&
                                    typeof(IServiceBehavior).GetTypeInfo().IsAssignableFrom(t) &&
                                    t.GetTypeInfo().GetCustomAttribute<ModuleNameAttribute>() == null
                                    && t.GetTypeInfo().GetCustomAttribute<DisableConventionalRegistrationAttribute>() == null).SingleInstance()
                        .AsImplementedInterfaces();
        ...
        /// <returns>IServiceBuilder.</returns>
        /// <exception cref="ArgumentNullException">builder</exception>
        public static IServiceCollection RegisterModules(this IServiceCollection services, params string[] virtualPaths)
        {
            var referenceAssemblies = ModuleHelper.GetAssemblies(virtualPaths);
            if (services == null)
            {
            ...
            foreach (var moduleAssembly in referenceAssemblies)
            {
                ModuleHelper.GetAbstractModules(moduleAssembly).ForEach(p =>
                {
                    ...
                    if (p.Enable)
                    {
                        p.ConfigureServices(new ServiceCollectionWrapper(services));
                    }
                });
            }
        ...
        public static IEnumerable<string> GetDataContractName(this IServiceBuilder builder, params string[] virtualPaths)
        {
            ...
            {
                namespaces.AddIfNotContains(assembly.GetTypes().Where(t => t.GetCustomAttribute<DataContractAttribute>() != null || t.GetCustomAttribute<SerializableAttribute>() != null).Select(n => n.Namespace));
            });
        ...
        /// <param name="builder">服务构建器</param>
        /// <returns>服务构建器</returns>
        public static IServiceBuilder UseSkyApmLogger(this IServiceBuilder builder)
        {
            builder.ContainerBuilder.RegisterType(typeof(NullSkyApmLogger)).As(typeof(ISkyApmLogger)).SingleInstance();
            return builder;
        }
    }
}
