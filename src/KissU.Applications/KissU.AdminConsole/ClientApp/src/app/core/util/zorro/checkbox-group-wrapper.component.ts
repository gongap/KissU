...
export class CheckboxGroup implements OnInit, AfterViewInit {
  ...
  private initDataSource() {
    if (!this.dataSource || !this.model) return;
    this.dataSource.forEach(t => {
      if (this.model.some(item => item === t.value)) t.checked = true;
    });
  }
  ...
  private addControl() {
    ...
    if (this.required) this.controlModel.control.setValidators(requiredValidator(this.dataSource));
