 ...
 * Crud弹出层树形编辑组件基类
 */
export abstract class DialogTreeEditComponentBase<TViewModel extends TreeViewModel>
  extends EditComponentBase<TViewModel>
  implements OnInit {
  /**
   * 父节点
   */
  ...
  protected getParent() {
    ...
    if (this.data && this.data.parentId) return { id: this.data.parentId, name: this.data.parentName };
    return null;
  }
  ...
  protected onSelectOk(instance) {
    if (!instance) return;
    if (!instance.getCheckedNode) return;
