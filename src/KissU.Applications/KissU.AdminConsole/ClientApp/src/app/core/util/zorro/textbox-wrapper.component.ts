...
@Component({
  ...
  template: `
    <nz-form-control
      [nzValidateStatus]="controlModel?.invalid && (controlModel?.dirty || controlModel.touched) ? 'error' : 'success'"
    >
      <input
        nz-input
        [id]="rawId"
        ...
        [maxlength]="maxLength"
      />
      <nz-form-explain *ngIf="controlModel?.invalid && (controlModel?.dirty || controlModel.touched)">{{
        getErrorMessage()
      }}</nz-form-explain>
    </nz-form-control>
  `,
...
export class TextBox extends FormControlWrapperBase implements OnInit {
   ...
   */
  getErrorMessage() {
    if (!this.controlModel) return '';
    if (this.controlModel.hasError('required')) return this.requiredMessage;
    ...
    if (this.controlModel.hasError('email')) return this.emailMessage || MessageConfig.emailMessage;
    if (this.controlModel.hasError('pattern')) return this.patterMessage;
    return '';
  }
