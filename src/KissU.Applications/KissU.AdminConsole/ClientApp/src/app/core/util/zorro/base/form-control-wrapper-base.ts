...
export class FormControlWrapperBase implements AfterViewInit, OnDestroy {
   ...
   * 控件模型
   */
  @ViewChild('controlModel', null) controlModel: NgModel;
  /**
   * 组件元素
   */
  @ViewChild(forwardRef(() => 'control'), null) element: ElementRef;
   ...
   */
  registerToRow() {
    if (this.row) this.row.register(this);
  }
   ...
   */
  unRegisterFromRow() {
    if (this.row) this.row.unRegister(this);
  }
   ...
   */
  isValid() {
    if (!this.controlModel) return false;
   ...
   */
  getNativeElement() {
    if (this.element) return this.element.nativeElement;
    return null;
  }
