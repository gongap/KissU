...
export class TreeTable<T extends IKey> extends Table<T> {
  ...
  isShowRadio(node) {
    ...
    if (!this.checkLeafOnly) return true;
  ...
  isShowText(node) {
    ...
    if (!this.multiple && this.checkLeafOnly && !this.isLeaf(node)) return true;
    return false;
  }
  ...
  private toggleParents(node) {
    ...
    if (isAllChecked) this.checkedSelection.select(parent);
    else this.checkedSelection.deselect(parent);
  ...
  protected loadData(result) {
    ...
    if (this.expandAll) this.dataSource.forEach(item => (item.expanded = true));
  }
}
