...
@Component({
  ...
  template: `
    <nz-upload
      ...
      [nzAccept]="accept"
      [nzFilter]="customFilters ? customFilters : filters"
...
export class Upload implements AfterViewInit {
   ...
   * 上传组件实例
   */
  @ViewChild(forwardRef(() => NzUploadComponent), null) instance: NzUploadComponent;
  /**
   * 文本框组件模型，用于验证
   */
  @ViewChild(forwardRef(() => NgModel), null) controlModel: NgModel;
   ...
   */
  getPreviewHandler() {
    if (this.listType === 'text') return undefined;
   ...
   */
  isShowButton() {
    if (this.showButton === false) return false;
    if (!this.totalLimit) return this.showButton;
  ...
  handleChange(data: { file; fileList; event; type }) {
    if (!data || !data.file || !this.uploadService) return;
  ...
  filters: UploadFilter[] = [
    {
      name: 'type',
      fn: (files: UploadFile[]) => {
        if (!this.instance || !this.instance.nzAccept) return files;
        ...
        if (invalidFiles && invalidFiles.length > 0) util.message.warn(this.getMessageByType(invalidFiles));
        return validFiles;
      },
    },
    {
      name: 'size',
      fn: (files: UploadFile[]) => {
        if (!this.instance || this.instance.nzSize === 0) return files;
        let validFiles = files.filter(file => file.size / 1024 <= this.instance.nzSize);
        let invalidFiles = util.helper.except(files, validFiles);
        if (invalidFiles && invalidFiles.length > 0) util.message.warn(this.getMessageBySize(invalidFiles));
        return validFiles;
      },
    },
    {
      name: 'limit',
      fn: (files: UploadFile[]) => {
        if (!this.instance || !this.instance.nzMultiple || this.instance.nzLimit === 0) return files;
        let validFiles = files.slice(-this.instance.nzLimit);
        let invalidFiles = util.helper.except(files, validFiles);
        if (invalidFiles && invalidFiles.length > 0) util.message.warn(this.getMessageByLimit(invalidFiles));
        return validFiles;
      },
    },
  ];
  ...
  handlePreview = (file: UploadFile) => {
    if (!file) return;
    if (!util.helper.isImage(file.url)) return;
