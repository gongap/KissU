...
import { genArr, genData, genContent, genMp } from './utils';
...
function domainList(queryString: any) {
  if (DATA.domain) return genPage('domain', queryString, 'subject');
...
function domainLogList(queryString: any) {
  if (DATA.domainLog) return genPage('domainLog', queryString, 'domain');
...
function dnsGroup() {
  if (DATA.dnsGroup) return DATA.dnsGroup;
  DATA.dnsGroup = [{ name: '全部域名', id: 0, count: 2 }, { name: 'test', id: ++MOCKID, count: 0 }];
...
function dnsSettingList(queryString: any) {
  if (DATA.dnsSetting) return genPage('dnsSetting', queryString, 'rR');
...
function apisList(queryString: any) {
  if (DATA.apis) return genPage('apis', queryString, 'name');
  const res: any[] = new Array(3).fill({}).map(() => ({
    id: MOCKID++,
...
function helpCategories() {
  if (DATA.help_categories) return DATA.help_categories;
  let point = 0;
...
function helpCategoriesGet(id: number) {
  const cate = (helpCategories() as any[])
    .reduce((p: any[], c: any) => (p = p.concat(c.list)), [])
    .find(w => w.id === id);
  ...
  return {
    ...
    recommended: new Array(Random.integer(0, 5))
      .fill({})
      .map((i, idx) => ({ id: idx + 1, title: Random.ctitle(5, 10) })),
  };
}
...
function messageInnerList(queryString: any) {
  ...
  if (DATA.notifications) return genPage('notifications', queryString, 'type', callback);
...
function userDefaultAvatar() {
  if (DATA.userDefaultAvatar) return DATA.userDefaultAvatar;
...
function financeList(queryString: any) {
  if (DATA.finance) return genPage('finance', queryString, 'subject');
  const res: any[] = new Array(11).fill({}).map(() => ({
    id: MOCKID++,
