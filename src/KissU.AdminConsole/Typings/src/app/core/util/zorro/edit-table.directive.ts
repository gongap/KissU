...
export class EditTableDirective {
  ...
  private handleEditChange(rowId) {
    if (this.creationIds.some(id => id === rowId)) return;
    if (this.updateIds.some(id => id === rowId)) return;
  ...
  addRow(options: {
    ...
    if (!row) return;
    this.initRow(row);
    if (options.before && !options.before(row)) return;
    if (this.creationIds.some(id => id === row.id)) return;
   ...
   */
  clickEdit(rowId) {
    if (this.dblClickStartEdit && !this.currentRow) return;
    this.editRow(rowId);
  }
  ...
  save(options?: {
    ...
    if (options.before && !options.before(data)) return;
  ...
  private isDirty(data, handler: (data) => boolean) {
    if (data.creationList && data.creationList.length > 0) return true;
    if (data.updateList && data.updateList.length > 0) return true;
    if (data.deleteList && data.deleteList.length > 0) return true;
  ...
  private processData(data) {
    if (data.creationList) data.creationList = util.helper.toJson(data.creationList);
    if (data.updateList) data.updateList = util.helper.toJson(data.updateList);
    if (data.deleteList) data.deleteList = util.helper.toJson(data.deleteList);
  }
}
