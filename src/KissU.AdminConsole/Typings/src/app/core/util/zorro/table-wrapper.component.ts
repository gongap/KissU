...
export class Table<T extends IKey> implements OnInit {
  ...
  private initPage() {
    ...
    if (this.pageSizeOptions && this.pageSizeOptions.length > 0) this.queryParam.pageSize = this.pageSizeOptions[0];
  }
  ...
  query(options?: {
    ...
    if (options.pageIndex) param.page = options.pageIndex;
    util.webapi
      .get<any>(url)
      .param(param)
  ...
  checkIds(ids) {
    ...
    list.forEach(item => {
      if (this.checkedSelection.isSelected(item)) return;
      this.checkedSelection.select(item);
    });
  }
  ...
  private getSortKey(sortKey, sortValue) {
    if (!sortValue) return this.sortKey;
    if (sortValue === 'ascend') return sortKey;
    return `${sortKey} desc`;
  }
  ...
  isMasterChecked() {
    return (
      this.checkedSelection.hasValue() &&
      this.isAllChecked() &&
  ...
  addRow(row) {
    if (!row) return;
    if (this.dataSource.some(t => t.id === row.id)) return;
  ...
  removeRows(ids?: string[]) {
    if (!ids || ids.length === 0) ids = this.getChecked().map(value => value.id);
    if (!ids || ids.length === 0) {
      util.message.warn(config.deleteNotSelected);
      return null;
    }
