...
namespace KissU.DotNetty.Udp
{
    ...
    internal class TcpServiceHost : ServiceHostAbstract
    {
        /// <summary>
        /// Initializes a new instance of the <see cref="TcpServiceHost" /> class.
        /// </summary>
        ...
        public TcpServiceHost(Func<EndPoint, Task<IMessageListener>> messageListenerFactory,
        ...
        public override async Task StartAsync(string ip, int port)
        {
            ...
            _serverMessageListener =
                await _messageListenerFactory(
                    new IPEndPoint(IPAddress.Parse(ip), AppConfig.ServerOptions.Ports.TcpPort));
            _serverMessageListener.Received += async (sender, message) =>
            {
                await MessageListener.OnReceived(sender, message);
            };
