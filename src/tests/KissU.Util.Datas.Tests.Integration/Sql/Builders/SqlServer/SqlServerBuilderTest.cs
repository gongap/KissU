...
namespace KissU.Util.Datas.Tests.Integration.Sql.Builders.SqlServer
{
    ...
    public partial class SqlServerBuilderTest
    {
        ...
        public void TestIsDeletedFilter_1()
        {
            ...
            //执行
            _builder.Select<Sample5>(t => t.StringValue).From<Sample5>("s").Join<Sample2>("s2")
                .On<Sample5, Sample2>((l, r) => l.IntValue == r.IntValue);
        ...
        public void TestIsDeletedFilter_3()
        {
            ...
            result.AppendLine(
                "Left Join [Sample7] As [s7] On [s6].[IntValue]=[s7].[IntValue] And [s7].[IsDeleted]=@_p_2 ");
            result.AppendLine(
                "Right Join [Sample8] As [s8] On [s7].[IntValue]=[s8].[IntValue] And [s8].[IsDeleted]=@_p_3 ");
