...
namespace KissU.BackgroundServer.Runtime.Implementation
{
    ...
    public class DefaultBackgroundServiceEntryProvider : IBackgroundServiceEntryProvider
    {
        ...
        public DefaultBackgroundServiceEntryProvider(IServiceEntryManager serviceEntryProvider,
        ...
        public IEnumerable<BackgroundServiceEntry> GetEntries()
        {
            ...
            if (_backgroundServiceEntries == null)
            {
                ...
                if (_backgroundServiceEntries.Any())
                {
                    _logger.LogInformation($"发现了{_backgroundServiceEntries.Count}个后台托管服务：");
                    _backgroundServiceEntries.ForEach(p => _logger.LogInformation(p.Type.FullName));
                }
            }
        ...
        public BackgroundServiceEntry CreateServiceEntry(Type service, string defaultRouteTemplate= "api/{Service}/{Async}")
        {
