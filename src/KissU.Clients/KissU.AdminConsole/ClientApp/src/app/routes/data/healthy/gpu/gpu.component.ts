...
export class DataHealthyGPUComponent implements OnInit, OnDestroy {
  ...
  private init(container: HTMLElement) {
    ...
    chart
      .area()
      .position('time*value')
      .tooltip('type*value', (type, val) => ({
        name: type === 'x1' ? 'GPU1' : 'GPU2',
        value: this.genUnit(val),
      }))
      .color('type', ['#91d5ff', '#69c0ff'])
      .opacity(0.2);
