...
namespace KissU.ServiceDiscovery.Zookeeper
{
    ...
    /// Implements the <see cref="KissU.CPlatform.Routing.Implementation.ServiceRouteManagerBase" />
    /// Implements the <see cref="System.IDisposable" />
    /// </summary>
    /// <seealso cref="KissU.CPlatform.Routing.Implementation.ServiceRouteManagerBase" />
    ...
    public class ZooKeeperServiceRouteManager : ServiceRouteManagerBase, IDisposable
    {
        ...
        public async Task ChildrenChange(string[] oldChildrens, string[] newChildrens)
        {
            ...
            {
                _logger.LogDebug($"需要被删除的路由节点：{deletedChildren}");
            }
           
            foreach (var createdChildren in createdChildrens)
            {
                _logger.LogDebug($"需要被添加的路由节点：{createdChildren}");
            }
