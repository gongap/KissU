...
using KissU.Util.Biz.Payments.Alipay.Services.CsvMappings;
using TinyCsvParser;
...
namespace KissU.Util.Biz.Payments.Alipay.Services
{
    ...
    public class AlipayDownloadBillService : AlipayServiceBase<AlipayDownloadBillRequest>, IAlipayDownloadBillService
    {
        ...
        private async Task<List<AlipayBillInfo>> Download(string url)
        {
            ...
            using (var zip = new ZipArchive(new MemoryStream(response), ZipArchiveMode.Read, false,
        ...
        /// 创建结果
        /// </summary>
        protected virtual AlipayDownloadBillResult CreateResult(AlipayResult result, string url,
            List<AlipayBillInfo> bills)
        {
