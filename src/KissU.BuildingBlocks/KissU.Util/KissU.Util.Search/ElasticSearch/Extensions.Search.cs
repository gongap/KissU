...
namespace KissU.Util.Search.ElasticSearch
{
    ...
    public static partial class Extensions
    {
        ...
        public static async Task<List<TResult>> GetAllAsync<TResult>(this IElasticSearch source, string index = null)
            where TResult : class
        {
            source.CheckNull(nameof(source));
            index = GetIndex<TResult>(index);
            var result = await source.GetClient()
                .SearchAsync<TResult>(s => s.Index(index).Size(10000).Query(q => q.MatchAll()));
        ...
        public static Search<TResult> Search<TResult>(this IElasticSearch source, IQueryParameter query)
            where TResult : class
        {
            source.CheckNull(nameof(source));
