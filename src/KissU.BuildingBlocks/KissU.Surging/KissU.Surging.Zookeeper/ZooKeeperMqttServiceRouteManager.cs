...
using KissU.Surging.Zookeeper.WatcherProvider;
...
namespace KissU.Surging.Zookeeper
{
    ...
    /// Implements the <see cref="KissU.Surging.CPlatform.Mqtt.Implementation.MqttServiceRouteManagerBase" />
    /// Implements the <see cref="System.IDisposable" />
    /// </summary>
    /// <seealso cref="KissU.Surging.CPlatform.Mqtt.Implementation.MqttServiceRouteManagerBase" />
    ...
    public class ZooKeeperMqttServiceRouteManager : MqttServiceRouteManagerBase, IDisposable
    {
        ...
        public ZooKeeperMqttServiceRouteManager(ConfigInfo configInfo, ISerializer<byte[]> serializer,
            ...
            ILogger<ZooKeeperMqttServiceRouteManager> logger, IZookeeperClientProvider zookeeperClientProvider) : base(
            stringSerializer)
        {
        ...
        public override async Task SetRoutesAsync(IEnumerable<MqttServiceRoute> routes)
        {
            ...
            if (serviceRoutes.Count() > 0)
            {
                foreach (var route in routes)
                {
                    var serviceRoute = serviceRoutes.Where(p => p.MqttDescriptor.Topic == route.MqttDescriptor.Topic)
                        .FirstOrDefault();
