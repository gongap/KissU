...
namespace KissU.Modules.IdentityServer.Data.Repositories
{
    ...
    public class ApiResourceRepository : RepositoryBase<ApiResource>, IApiResourceRepository
    {
        ...
        {
            var entity = await UnitOfWork.Set<ApiResourceScope>().Where(x => x.Id == id).SingleAsync();
        ...
        {
            var entity = await UnitOfWork.Set<ApiResourceSecret>().Where(x => x.Id == id).SingleAsync();
