...
namespace KissU.Modules.GreatWall.Application.Implements
{
    ...
    /// 身份资源查询服务
    /// </summary>
    public class QueryIdentityResourceAppService : QueryServiceBase<ResourcePo, IdentityResourceDto, ResourceQuery>,
        IQueryIdentityResourceAppService
    {
        /// <summary>
        /// 初始化身份资源服务
        /// </summary>
        ...
        public async Task<List<IdentityResourceDto>> GetResources(List<string> uri)
        {
            ...
            var resources =
                await ResourceStore.FindAllAsync(t => uri.Contains(t.Uri) && t.Type == ResourceType.Identity);
            if (resources == null)
                return new List<IdentityResourceDto>();
