...
namespace KissU.Modules.GreatWall.Data.Repositories
{
    ...
    /// 身份资源仓储
    /// </summary>
    public class IdentityResourceRepository : CompactRepositoryBase<IdentityResource, ResourcePo>,
        IIdentityResourceRepository
    {
        ...
        {
            var exists =
                await _store.ExistsAsync(t => t.Uri == identityResource.Uri && t.Type == ResourceType.Identity);
            return exists == false;
        }
        ...
        public async Task<bool> CanUpdateAsync(IdentityResource identityResource)
        {
            var exists = await _store.ExistsAsync(t =>
                t.Id != identityResource.Id && t.Uri == identityResource.Uri && t.Type == ResourceType.Identity);
            return exists == false;
        }
