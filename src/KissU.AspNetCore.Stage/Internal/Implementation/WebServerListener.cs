...
namespace KissU.AspNetCore.Stage.Internal.Implementation
{
    ...
    public class WebServerListener : IWebServerListener
    {
        ...
        void IWebServerListener.Listen(WebHostContext context)
        {
            ...
            if (httpPorts != null)
            {
                if (CPlatform.AppConfig.ServerOptions.Protocols != nameof(CommunicationProtocol.Http) && CPlatform.AppConfig.ServerOptions.Ports.HttpPort.HasValue)
                {
                    httpPorts = httpPorts.Except(new List<string>{CPlatform.AppConfig.ServerOptions.Ports.HttpPort.ToString()}).ToArray();
                }
