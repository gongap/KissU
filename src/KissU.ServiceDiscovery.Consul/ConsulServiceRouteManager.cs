...
namespace KissU.ServiceDiscovery.Consul
{
    ...
    public class ConsulServiceRouteManager : ServiceRouteManagerBase, IDisposable
    {
        ...
        private async Task ChildrenChange(string[] oldChildrens, string[] newChildrens)
        {
            ...
            {
                _logger.LogTrace($"需要被删除的路由节点：{deletedChildren}");
            }
           
            foreach (var createdChildren in createdChildrens)
            {
                _logger.LogTrace($"需要被添加的路由节点：{createdChildren}");
            }
            ...
            {
                _logger.LogInformation($"服务路由数据更新成功，删除节点数：{deletedChildrens.Length}，添加节点数：{createdChildrens.Length}。");
            }
        }
