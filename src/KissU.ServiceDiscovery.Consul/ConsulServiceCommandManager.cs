...
namespace KissU.ServiceDiscovery.Consul
{
    ...
    public class ConsulServiceCommandManager : ServiceCommandManagerBase, IDisposable
    {
        ...
        public async Task ChildrenChange(string[] oldChildrens, string[] newChildrens)
        {
            ...
            {
                _logger.LogTrace($"需要被删除的服务命令节点：{deletedChildren}");
            }
           
            foreach (var createdChildren in createdChildrens)
            {
                _logger.LogTrace($"需要被添加的服务命令节点：{createdChildren}");
            }
            ...
            {
                _logger.LogInformation($"服务命令数据更新成功，删除节点数：{deletedChildrens.Length}，添加节点数：{createdChildrens.Length}。");
            }
        }
    }
}