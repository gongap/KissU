...
namespace KissU.Convertibles.Implementation
{
    ...
    public class DefaultTypeConvertibleProvider : ITypeConvertibleProvider
    {
        ...
        public IEnumerable<TypeConvertDelegate> GetConverters()
        {
            ...
            //// 字典类型
            yield return DictionaryTypeConvert;
        }
        ...
        /// <param name="conversionType">Type of the conversion.</param>
        /// <returns>System.Object.</returns>
        private object DictionaryTypeConvert(object instance, Type conversionType)
        {
            try
            {
                if (instance == null || !TypeHelper.IsDictionary(instance.GetType(),out var keyType, out var valueType))
                {
                    return null;
                }
                
                return _serializer.Deserialize(_serializer.Serialize(instance), conversionType);
            }
            catch
            {
                return null;
