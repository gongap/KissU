...
namespace KissU.Util.Datas.Tests.Integration.Sql.Builders.SqlServer.Resolvers
{
    ...
    public class PredicateExpressionResolverTest
    {
        ...
        public PredicateExpressionResolverTest()
        {
            ...
            _resolver = new PredicateExpressionResolver(_dialect, new EntityResolver(), new EntityAliasRegister(),
                _parameterManager);
        }
        ...
        public void TestResolve_5()
        {
            ...
            Assert.Equal("([Email]=@_p_0 And [IntValue]=@_p_1 Or [DisplayValue]=@_p_2)",
                _resolver.Resolve(expression).GetCondition());
            Assert.Equal("a", _parameterManager.GetParams()["@_p_0"]);
            Assert.Equal(1, _parameterManager.GetParams()["@_p_1"]);
        ...
        {
            Expression<Func<Sample, bool>> expression = t => t.Email == "a" || t.IntValue == 1 && t.DisplayValue == "b";
            Assert.Equal("([Email]=@_p_0 Or [IntValue]=@_p_1 And [DisplayValue]=@_p_2)",
                _resolver.Resolve(expression).GetCondition());
            Assert.Equal("a", _parameterManager.GetParams()["@_p_0"]);
            Assert.Equal(1, _parameterManager.GetParams()["@_p_1"]);
            Assert.Equal("b", _parameterManager.GetParams()["@_p_2"]);
        ...
        {
            Expression<Func<Sample, bool>> expression = t =>
                (t.Email == "a" || t.IntValue == 1) && t.DisplayValue == "b";
            Assert.Equal("([Email]=@_p_0 Or [IntValue]=@_p_1) And [DisplayValue]=@_p_2",
                _resolver.Resolve(expression).GetCondition());
            Assert.Equal("a", _parameterManager.GetParams()["@_p_0"]);
            Assert.Equal(1, _parameterManager.GetParams()["@_p_1"]);
        ...
        {
            Expression<Func<Sample, bool>> expression = t =>
                t.Email == "a" || t.IntValue == 1 || t.Email == "b" || t.IntValue == 2;
            Assert.Equal("(([Email]=@_p_0 Or [IntValue]=@_p_1) Or ([Email]=@_p_2 Or [IntValue]=@_p_3))",
                _resolver.Resolve(expression).GetCondition());
            Assert.Equal("a", _parameterManager.GetParams()["@_p_0"]);
            Assert.Equal(1, _parameterManager.GetParams()["@_p_1"]);
            Assert.Equal("b", _parameterManager.GetParams()["@_p_2"]);
