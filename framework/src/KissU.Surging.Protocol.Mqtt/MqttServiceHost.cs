...
namespace KissU.Surging.Protocol.Mqtt
{
    /// <summary>
    /// MqttServiceHost.
    /// Implements the <see cref="KissU.Surging.CPlatform.Runtime.Server.Implementation.ServiceHostAbstract" />
    /// </summary>
    /// <seealso cref="KissU.Surging.CPlatform.Runtime.Server.Implementation.ServiceHostAbstract" />
        ...
        public override async Task StartAsync(string ip, int port)
        {
            ...
            _serverMessageListener =
                await _messageListenerFactory(new IPEndPoint(IPAddress.Parse(ip),
                    AppConfig.ServerOptions.Ports.MQTTPort));
        }
