...
namespace KissU.Util.Tests.Helpers
{
    ...
    public class EnumTest
    {
        ...
        {
            Assert.Equal(sample, EnumHelper.Parse<EnumSample>(memeber));
        }
        ...
        {
            AssertHelper.Throws<ArgumentNullException>(() => { EnumHelper.Parse<EnumSample>(member); }, "member");
        }
        ...
        {
            Assert.Equal(name, EnumHelper.GetName<EnumSample>(member));
        }
        ...
        {
            Assert.Equal(value, EnumHelper.GetValue<EnumSample>(member));
        }
        ...
        {
            Assert.Equal(description, EnumHelper.GetDescription<EnumSample>(member));
        }
        ...
        {
            var items = EnumHelper.GetItems<EnumSample>();
        ...
        {
            var items = EnumHelper.GetItems(typeof(EnumSample?));
            Assert.Equal(5, items.Count);
            Assert.Equal("A", items[0].Text);
            Assert.Equal(1, items[0].Value);
            Assert.Equal("D4", items[3].Text);
        ...
        {
            AssertHelper.Throws<InvalidOperationException>(() => { EnumHelper.GetItems<Sample>(); },
        ...
        {
            Assert.Equal(string.Empty, EnumHelper.GetName(typeof(Sample), 3));
        }
        ...
        {
            var names = EnumHelper.GetNames<EnumSample>().OrderBy(t => t).ToList();
        ...
        {
            AssertHelper.Throws<ArgumentNullException>(() => EnumHelper.GetValue<EnumSample>(null), "member");
            AssertHelper.Throws<ArgumentNullException>(() => EnumHelper.GetValue<EnumSample>(string.Empty), "member");
            AssertHelper.Throws<ArgumentNullException>(() => EnumHelper.GetValue<Sample>(string.Empty), "member");
        }
    }
}