...
namespace KissU.Util.Tests.Helpers
{
    ...
    public partial class StringTest
    {
        ...
        {
            Assert.Equal(result, StringObj.PinYin(input));
        }
        ...
        {
            Assert.Equal(result, StringObj.FirstLowerCase(value));
        }
        ...
        {
            Assert.Equal(result, StringObj.FirstUpperCase(value));
        }
        ...
        {
            Assert.Equal(result, StringObj.SplitWordGroup(value));
        }
        ...
        {
            Assert.Equal("1,2,3", StringObj.Join(new List<int> {1, 2, 3}));
            Assert.Equal("'1','2','3'", StringObj.Join(new List<int> {1, 2, 3}, "'"));
            Assert.Equal("123", StringObj.Join(new List<int> {1, 2, 3}, "", ""));
            Assert.Equal("\"1\",\"2\",\"3\"", StringObj.Join(new List<int> {1, 2, 3}, "\""));
            Assert.Equal("1 2 3", StringObj.Join(new List<int> {1, 2, 3}, "", " "));
            Assert.Equal("1;2;3", StringObj.Join(new List<int> {1, 2, 3}, "", ";"));
            Assert.Equal("1,2,3", StringObj.Join(new List<string> {"1", "2", "3"}));
            Assert.Equal("'1','2','3'", StringObj.Join(new List<string> {"1", "2", "3"}, "'"));
