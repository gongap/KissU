...
namespace KissU.Util.Tests.Helpers
{
    ...
    public class TimeTest : IDisposable
    {
        ...
        {
            Assert.Equal(new DateTime(1970, 01, 01, 12, 12, 12), TimeHelper.GetTimeFromUnixTimestamp(15132));
            Assert.Equal(new DateTime(2000, 12, 12, 12, 12, 12), TimeHelper.GetTimeFromUnixTimestamp(976594332));
            Assert.Equal(new DateTime(2014, 02, 18, 04, 24, 59), TimeHelper.GetTimeFromUnixTimestamp(1392668699));
        }
        ...
        {
            Assert.Equal(15132, TimeHelper.GetUnixTimestamp(new DateTime(1970, 01, 01, 12, 12, 12)));
            Assert.Equal(976594332, TimeHelper.GetUnixTimestamp(new DateTime(2000, 12, 12, 12, 12, 12)));
            Assert.Equal(1392668699, TimeHelper.GetUnixTimestamp(new DateTime(2014, 02, 18, 04, 24, 59)));
        }
        ...
        {
            TimeHelper.SetTime(Datetime1);
            Assert.Equal(Datetime1, TimeHelper.GetDateTime());
            TimeHelper.Reset();
            Assert.NotEqual(Datetime1, TimeHelper.GetDateTime());
        }
    }
}